"""
Canonical external HTTP contract models for GhostStream.

New wire shapes belong here, not in runtime/controller modules.
"""
...
class TranscodeRequest(BaseModel):
    ...
    subtitles: Optional[List[SubtitleTrack]] = Field(
        default=None,
        description="Subtitle tracks to mux into HLS stream",
    )
...
class TranscodeResponse(BaseModel):
    ...
    start_time: float = Field(
        default=0,
        description="Actual start time the transcode began at (may differ from requested for shared streams)",
    )
